		...
		switch ($real_type) {
			...
			case 'upload_file':
				...
				else
				{
					...
					if($is_image)
					{
						$file_url_anchor .= ' class="image-thumbnail"><img src="'.$file_url.'" style="height:50px">';
					}
					else
					{
			...
			else
			{
				...
				foreach($columns as $column)
				{
					...
					else
					{
                        ...
                        if(!strstr($field,$this->get_table().'.'))
                        {
                            $field=$this->get_table().'.'.$field;
                        }
						$this->or_like($field, $search_text);
					}
				}
			}
		}
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input .= $is_image ? " $image_class'><img src='".$file_url."' style='height:50px'>" : "' target='_blank'>$value";
		$input .= "</a> ";
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	{
		return $this->state_url('ajax_list').(!empty($_GET)?'?'.http_build_query($_GET):null);
	}
	...
	protected function getEditUrl($primary_key = null)
	{
		...
		else
        {
            ...
            {
                return $this->edit_link.$primary_key;
            }
        }
	}
	...
	protected function getDeleteUrl($state_info = null)
	{
		...
		else
        {
            ...
            {
                return $this->delete_link.$state_info->primary_key;
            }
        }
	}
...
//@todo ui_day, ui_month, ui_year has to be lang strings
	...
	{
        if($upload_dir[0]=='/')$upload_dir=substr($upload_dir,1);
